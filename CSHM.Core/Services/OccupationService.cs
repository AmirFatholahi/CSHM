...
{
    public class OccupationService : Repository<Occupation, OccupationViewModel>, IOccupationService
    {
        private readonly ILogWidget _log;
        private readonly IMapper _mapper;

        public OccupationService(DatabaseContext context, ILogWidget log, IMapper mapper) : base(context, log, mapper)
        {
            _log = log;
            _mapper = mapper;
        }

        public override ResultViewModel<OccupationViewModel> SelectAll(bool? activate, string filter = null, int? pageNumber = null, int pageSize = 20)
        {
            throw new NotImplementedException();
        }

        public List<ErrorViewModel> ValidationForm(OccupationViewModel entity)
        {
