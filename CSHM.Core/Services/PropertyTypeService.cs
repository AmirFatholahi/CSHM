...
{
    public class PropertyTypeService : Repository<PropertyType, PropertyTypeViewModel>, IPropertyTypeService
    {
        ...
        public PropertyTypeService(DatabaseContext context, ILogWidget log, IMapper mapper) : base(context, log, mapper)
        {
            _log = log;
            _mapper = mapper;
        }

        public override ResultViewModel<PropertyTypeViewModel> SelectAll(bool? activate, string filter = null, int? pageNumber = null, int pageSize = 20)
        {
            var result = new ResultViewModel<PropertyTypeViewModel>();
            try
            {
                IQueryable<PropertyType> items;
                Expression<Func<PropertyType, bool>> condition = x => string.IsNullOrWhiteSpace(filter);
        ...
        private List<ErrorViewModel> ValidationForm(PropertyType entity)
        {
