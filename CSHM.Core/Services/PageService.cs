...
public class PageService : Repository<Page, PageViewModel>, IPageService
{
    private readonly ILogWidget _log;
    private readonly IMapper _mapper;
    private readonly IExcelWidget _excel;

    public PageService(DatabaseContext context, ILogWidget log, IMapper mapper, IExcelWidget excel) : base(context, log, mapper)
    {
        _log = log;
        _mapper = mapper;
        _excel = excel;
    ...
    {
        PageViewModel result;
        var item = GetAll(true, x => x.Name.ToLower() == pageName.ToLower()).FirstOrDefault();
    ...
    /// <returns></returns>
    private List<ErrorViewModel> ValidateForm(Page entity)
    {
        ...
        if (entity.Priority <= 0)
        {
            ...
            {
                ErrorCode = Errors.Error930,
                ErrorMessage = string.Format(Messages.FieldIsRequired, "اولویت")
            });
        }
        ...
        if (!string.IsNullOrEmpty(entity.Path) && entity.Path.Length > 250)
        {
            result.Add(new ErrorViewModel()
            {
                ErrorCode = Errors.Error931,
                ErrorMessage = string.Format(Messages.FieldMaxLengthExceeded, "مسیر", 250)
            });
        }
        if (!string.IsNullOrEmpty(entity.Icon) && entity.Icon.Length > 50)
        {
            result.Add(new ErrorViewModel()
            {
                ErrorCode = Errors.Error931,
                ErrorMessage = string.Format(Messages.FieldMaxLengthExceeded, "آیکن", 50)
            });
        }
