...
public class NotificationTypeService : Repository<NotificationType, NotificationTypeViewModel>, INotificationTypeService
{
    private readonly ILogWidget _log;
    private readonly IMapper _mapper;
    private readonly IExcelWidget _excel;

    public NotificationTypeService(DatabaseContext context, ILogWidget log, IMapper mapper, IExcelWidget excel) : base(context, log, mapper)
    {
        _log = log;
        _mapper = mapper;
        _excel = excel;
    ...
    public override ResultViewModel<NotificationTypeViewModel> SelectAll(bool? activate, string? filter = null, int? pageNumber = null, int pageSize = 20)
    {
        var result = new ResultViewModel<NotificationTypeViewModel>();
        try
        {
            IQueryable<NotificationType> items;
            Expression<Func<NotificationType, bool>> condition = x => string.IsNullOrWhiteSpace(filter) || x.Title.Contains(filter);
    ...
    public HttpResponseMessage ExcelAll()
    {
        ...
        var list = _mapper.Map<List<NotificationTypeExcelModel>>(items);
    ...
    /// <returns></returns>
    private List<ErrorViewModel> ValidateForm(NotificationType entity)
    {
        ...
        //Max Length
        if (!string.IsNullOrEmpty(entity.Title) && entity.Title.Length > 50)
        {
            result.Add(new ErrorViewModel()
            {
                ErrorCode = Errors.Error931,
                ErrorMessage = string.Format(Messages.FieldMaxLengthExceeded, "عنوان", 50)
            });
        }
