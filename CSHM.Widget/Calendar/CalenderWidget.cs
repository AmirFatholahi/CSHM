...
namespace CSHM.Widget.Calendar;
...
{ 
    ...
    public static DateTime ToGregDateTime(string jalaliDate)
    {
        ...
        if (!string.IsNullOrEmpty(jalaliDate))
        {
            ...
            var time = jalaliDate.Split("|");
            if (time.Length > 1)
            {
                var timeArray = time[1].Split(":");
                result = new DateTime(year, month, day, timeArray[0].ToInt(), timeArray[1].ToInt(), timeArray[2].ToInt(), new System.Globalization.PersianCalendar());
            }
            else
            {
    ...
    {
        bool result;
        var gregBirthdate = CalenderWidget.ToGregDateTime(birthdate);
